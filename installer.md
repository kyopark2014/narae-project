...
```python
# 프로젝트 전용
project_name = "narae-project"        # 프로젝트 이름 (최소 3자)
...
| `role-agent-for-narae-project-{region}` | 프로젝트 전용 | Strands Agent용 역할 |
| `role-agentcore-memory-for-narae-project-{region}` | 프로젝트 전용 | AgentCore Memory용 역할 |
| `role-agentcore-gateway-websearch-for-narae-project` | 프로젝트 전용 | AgentCore Web Search Gateway용 역할 |

### 3. OpenSearch Serverless (공유)
...
### 6. AgentCore Web Search Gateway (공유 가능)
...
프로젝트별로 독립적으로 유지되는 리소스는 **Agent IAM 역할**, **AgentCore Memory 역할**입니다.
...
```
[1/5] S3 버킷 생성 (공유 버킷, 없으면 생성)
       ↓
[2/5] IAM 역할 생성
       • Knowledge Base / Agent / AgentCore Memory 역할
       • AgentCore Web Search Gateway 역할 및 Gateway 생성
       ↓
[3/5] OpenSearch Serverless 컬렉션 생성 (공유, 없으면 생성)
       ↓
[4/5] Bedrock Knowledge Base 생성 (공유, 없으면 생성)
       ↓
[5/5] CloudFront 배포 생성 (S3 오리진, 없으면 생성)
       ↓
